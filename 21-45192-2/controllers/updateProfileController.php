...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['username'])) {
        $errors['username'] = "Please fill up your Name";
    } else {
   ...
   if ($result->num_rows === 1) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("UPDATE user SET name = ?, username = ?, address = ?, email = ?, phone = ? WHERE uid = ?");
        $stmt->bind_param("sssssi", $fullName, $username, $address, $email, $phone, $user_id);
